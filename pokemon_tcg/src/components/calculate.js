...
export async function calculateAverageValues() {
  try {
  ...
  try {
    ...
    for (const rarity of db) {
      ...
      if (values.length > 0) {
        ...
        console.log(`Average Reverse Holofoil Value for ${rarity}:`, averageReverseHolofoil);
...
async function finalCalc(array) {
  ...
  console.log("Final calculated value:", total);
}
